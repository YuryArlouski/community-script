...
#          7.0.47 - 01/25/2016 - Corey Swertfager:
#                   * Now removes:
#                        /usr/local/bin/npfx
#                   * Changed "-depth 1" to "-mindepth 1 -maxdepth 1" in some
#                     find calls.
#                   * Removed duplicate lookups when only generating a list of
#                     installed files of the following:
#                        /Library/Application Support/Symantec
#                        /Library/Application Support/Symantec/ErrorReporting
#          7.0.48 - 03/23/2016 - Corey Swertfager:
#                   * To avoid potential hangs, kextstat is no longer run (Etrack 3925328).
#                   * Added FilesThatDoNotRequireRebootPattern variable to help determine
#                     when a reboot is required in lieu of using kextstat (Etrack 3925328).
#                   * Backs up some logs to /private/var/tmp/SymantecDiagnosticLogs prior
#                     to deleting them if this program's name is not in listed in variable
#                     ListOfProgramsThatShouldNotBackUpLogs (Etrack 3912399).
#                   * Now removes:
#                        /tmp/com.symantec.NFM.liveupdate.reboot
#          7.0.49 - 05/02/2016 - Corey Swertfager:
#                   * Now removes:
...
#          7.0.50 - 08/02/2016 - Corey Swertfager:
#                   * Now excludes removal of /var/db/receipts/com.symantec-it.*
#                     (Etrack 3980221).
#          7.0.51 - 09/06/2016 - Corey Swertfager:
#                   * Now removes (Etrack 3968791):
#                        /Library/Logs/SymCommP*
#                        {each user's home directory}/Library/Logs/SymCommP*
#          7.0.52 - 09/12/2016 - Corey Swertfager:
#                   * Now removes (Etrack 3995440):
#                        /Applications/Symantec Endpoint Protection Cloud.app
#          7.0.53 - 10/20/2016 - Corey Swertfager:
#                   * Updated help for -e option.
#          7.0.54 - 02/01/2017 - Corey Swertfager:
#                   * Removed Watermark for ftp zip posting.
#          7.0.55 - 02/07/2017 - Corey Swertfager:
#                   * Now removes (Etrack 4060698):
#                        /Library/Extensions/SymXIPS.kext
#                        /System/Library/Extensions/SymXIPS.kext
#                   * Now removes Norton WiFi Privacy (NWP) files based on
#                     clean_norton.sh.
#          7.0.56 - 03/28/2017 - Corey Swertfager:
#                   * Now removes IPUA logs (Etrack 4074658):
#                        /Library/Logs/.ipuaint
#                        /Library/Logs/.ipualog
#                        /Library/Logs/SymaIpua*
#                        /Library/Logs/SymIpua*
#                        {each user's home directory}/Library/Logs/.ipuaint
#                        {each user's home directory}/Library/Logs/.ipualog
#                        {each user's home directory}/Library/Logs/SymaIpua*
#                        {each user's home directory}/Library/Logs/SymIpua*
#          7.0.57 - 03/30/2017 - Corey Swertfager:
#                   * Updated DeleteSymantecLoginItems() function to remove the
#                     following login items:
#                        Norton AntiVirus
#                        Norton WiFi Privacy
#          7.0.58 - 04/12/2017 - Corey Swertfager:
#                   * Adjusted pattern matching for -u option to fix an issue
#                     where pattern with [ was treated as bracket expression.
#          7.0.59 - 04/12/2017 - Corey Swertfager:
#                   * Updated DeleteSymantecLoginItems() function to remove the
#                     following login item:
#                        Norton WiFi Privacy.app
#                   * Updated DeleteSymantecLoginItems() function to address an
#                     error on OS 10.7 where regular expression was too large.
#          7.0.60 - 06/07/2017 - Corey Swertfager:
#                   * Now removes:
#                        /Library/Preferences/group.symantec*
#                        {each user's home directory}/Library/Preferences/group.symantec*
#          7.0.61 - 06/16/2017 - Corey Swertfager:
#                   * Now removes (Etrack 4100691):
#                        /Library/Preferences/group.com.symantec*
#                        {each user's home directory}/Library/Preferences/group.com.symantec*
#          7.0.62 - 09/06/2017 - Corey Swertfager:
#                   * Now removes:
#                        /var/tmp/nis_*_Data2.chk
#          7.0.63 - 10/09/2017 - Corey Swertfager:
#                   * Added DeleteDefaults() function.
#                   * Now removes defaults for group.symantec.nortonclean (Etrack 4132453).
#          7.0.64 - 10/16/2017 - Corey Swertfager:
#                   * Now removes:
#                        /usr/local/lib/libNUCoverFlow*
#          7.0.65 - 10/16/2017 - Corey Swertfager:
#                   * Added /usr/local/lib/libNUCoverFlow* to ReadMe.txt.
#          7.0.66 - 11/22/2017 - Corey Swertfager:
#                   * Now removes:
#                        /Library/Logs/EPMPTestLogs.log
#                        {each user's home directory}/Library/Logs/EPMPTestLogs.log
#          7.0.67 - 12/06/2017 - Corey Swertfager:
#                   * Updated warning above and in help.
#          7.0.68 - 01/05/2018 - Corey Swertfager:
#                   * Now passes -u to RemoveItem for Firefox and Safari browser
#                     extensions (Etrack 4147959).
#          7.0.69 - 01/11/2018 - Corey Swertfager:
#                   * Now shows begin and end time of predelete script runs.
#                   * Now shows predelete script exit code.
#                   * Predelete script output is now verbose when the -e option
#                     is passed.
#          7.0.70 - 01/12/2018 - Corey Swertfager:
#                   * Now uses System Events to restart computer instead of using
#                     reboot command when OS version is 10.2 or later.
#          7.0.71 - 02/22/2018 - Davee Nguyen:
#                   * Internal update.
#          7.0.72 - 03/02/2018 - Davee Nguyen:
#                   * Internal update.
#          7.0.73 - 03/22/2018 - Corey Swertfager:
#                   * Added -E option to suppress predelete output.
#                   * Made -e option (show predelete output) the default (Etrack 4168665).
#          7.0.74 - 03/24/2018 - Corey Swertfager:
#                   * Added --skip-symquald to call of any predelete script that uses
#                     that option to prevent symquald submission (Etrack 4168665).
#          7.0.75 - 07/03/2018 - Corey Swertfager:
#                   * Added processing of Data2.chk for -u option (Jira MACSYS-3932).
#          7.0.76 - 08/16/2018 - Corey Swertfager:
#                   * Now removes Symantec/Norton apps found in each user's
#                     Downloads folder (MACSYS-4013).
#          7.0.77 - 08/24/2018 - Corey Swertfager:
#                   * Internal update.
#          7.0.78 - 08/31/2018 - Corey Swertfager:
#                   * Now removes:
#                        {each user's home directory}/Library/Logs/SymMigration.log
#                        /Library/Logs/SymMigration.log
#                        /var/tmp/NFM
#          7.0.79 - 09/04/2018 - Corey Swertfager:
#                   * No longer removes when this script is named SymantecRemovalTool (MACSYS-4572):
#                        /var/tmp/NFM
#          7.0.80 - 09/11/2018 - Corey Swertfager:
#                   * Internal update.
#          7.0.81 - 09/28/2018 - Corey Swertfager:
#                   * Now passes -u to RemoveItem for:
#                        {each user's home directory}/Application Support
#                        {each user's home directory}/Library/Preferences/group.com.symantec*
#                        {each user's home directory}/Library/Preferences/group.symantec*
#                   * Now passes -E to RemoveItem for:
#                        {each user's home directory}/Application Support
#          7.0.82 - 10/15/2018 - Corey Swertfager:
#                   * No longer removes system profile com.symc.enroll when this
#                     script is named SymantecRemovalTool (MACSYS-4985):
#          7.0.83 - 01/14/2019 - Corey Swertfager:
#                   * Removed extra 7.0.81 line in History.
#          7.0.84 - 02/07/2019 - Corey Swertfager:
#                   * Now removes the following unless this script is named
#                     SymantecRemovalTool:
#                        /etc/symantec/mid.dat
#          7.0.85 - 04/15/2019 - Corey Swertfager:
#                   * Updated RemoveCrashReporterLogs() function to remove logs in:
#                        {each user's home directory}/Library/Logs/Crashes
#                        /Library/Logs/Crashes
#          7.0.86 - 04/16/2019 - Corey Swertfager:
#                   * Corrected history entry for previous version.
#                   * Updated log folders and Downloads items listed in ReadMe.txt.
#          7.0.87 - 06/24/2019 - Corey Swertfager:
#                   * Added support for multiple Silo directories.
#          7.0.88 - 06/24/2019 - Corey Swertfager:
#                   * Now removes:
#                        /Library/Services/NortonDocktile.bundle
#          7.0.89 - 06/28/2019 - Corey Swertfager:
#                   * Now removes:
#                        /tmp/com.symantec.MES.liveupdate.reboot
#          7.0.90 - 07/03/2019 - Corey Swertfager:
#                   * Now removes:
#                        /Applications/ApplicationNorton.app
#          7.0.91 - 07/13/2019 - Corey Swertfager:
#                   * Now removes:
#                        /var/tmp/Data2.chk
#          7.0.92 - 08/28/2019 - Corey Swertfager:
#                   * Added /var/tmp/com.symantec.mes.patcher.log
#                     to ItemsThatShouldRemainAfterUninstall.
#          8.0.0 - 09/06/2019 - Corey Swertfager:
#                  * Added removal prevention when running OS 10.15 or later
#                    via ProcessAppRemoval() function.
